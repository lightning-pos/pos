...
pub fn libsql_from_row_derive(input: TokenStream) -> TokenStream {
        ...
        if let Some(inner_type) = extract_option_inner_type(field_type) {
            ...
            field_conversions.push(quote! {
                let #field_name = match row.get_value(#i as i32) {
                    ...
                    Ok(value) => Some(<#inner_type as crate::adapters::outgoing::database::FromLibsqlValue>::from_libsql_value(value)?),
                    Err(_) => None,
                };
            });
        } else {
            ...
            field_conversions.push(quote! {
                let #field_name = <#field_type as crate::adapters::outgoing::database::FromLibsqlValue>::from_libsql_value(row.get_value(#i as i32)?)?;
            });
        }
    ...
    let gen = quote! {
        impl crate::adapters::outgoing::database::FromRow<libsql::Row> for #struct_name {
